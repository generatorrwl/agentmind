...
AgentMind 中影响长期行为和知识结构的重要设定，必须经过充分讨论和调研，不能由 agent 在一次输出里草率决定。

这类设定包括但不限于：

- 项目目标、长期目标和阶段性目标。
- Project wiki schema：分类体系、页面类型、frontmatter、命名规则、source class、index/log 规则。
- Project-specific maintenance skill：如何 ingest/query/lint/promote，以及哪些内容可以进入 fixed knowledge。
- Capability/skill promotion policy：什么行为可以沉淀成 reusable skill，什么必须保持为临时 runbook 或 proposal。
- 高影响 memory 和团队/用户工作偏好。

原则：**设定越准确，后续污染、返工和错误泛化越少。** 因此 agent 在创建或修改这些设定前，应执行 discovery workflow：

1. 先询问用户是否已有 schema、目标、分类体系、团队约定或参考项目。
2. 如果没有，先读取当前 workspace 的 README、docs、源码结构、历史对话、已有 wiki/skills/tools 和 open work。
3. 对项目所在业务/domain 做必要外部调研，参考类似项目的 wiki/schema/skill 组织方式。
...
AgentMind 不应把 Codex/Claude 内部 goal/plan 机制作为产品依赖。跨 harness 的目标、计划、schema discovery 状态和用户决策应以本地文件形式持久化，例如 work items、extraction packets、schema candidates、research notes 和 decision logs。

用户必须显式授权 AgentMind 初始化和高影响 Project Design，但不应被要求记住内部 CLI。初始化后，Agent 应通过入口指令、`doctor`、`status` 或 packet 发现缺少 Project Profile、Wiki Schema、Project Extraction Skill、Project Workflow Skill，并用自然语言建议用户是否启动 Project Design。只有用户确认后，Agent 才调用对应 CLI 生成 packet 和候选方案。
...
Project schema 和 project objectives 是 AgentMind 的高影响资产，必须遵循“先问用户、再读 workspace、再做调研、再提 proposal、再确认”的原则。

同一原则也适用于 project-specific extraction/workflow skills。它们不是 profile 自动生成的附属字段，也不是 Memory 直接生成的内容，而是通过 Project Design 流程与用户讨论后产生的高影响资产。Profile 的职责是引用已确认的 schema 和 project skills，并记录项目类型、目标、source priority、Not Now 和用户确认过的决策。

Project Design 的触发不应依赖用户记住 `agentmind project design`。当 AgentMind 已初始化但缺少 profile/schema/project skills 时，entry flow 和 `doctor/status` 应提示 Agent 向用户建议 Project Design；用户确认前不得自动生成最终 schema 或启用 project skills。

第一版应支持一个 schema discovery packet，而不是让 agent 直接生成最终 wiki 结构。推荐中间文件：
...
`agentmind-extraction` skill 应规定 schema discovery 的工作法：

- 询问用户是否已有 schema/目标/参考项目。
- 读取 project sources、history、episodes、existing wiki 和 skills。
- 必要时做网络调研，寻找同类项目的 wiki、docs、skill、runbook 和 taxonomy。
- 给出候选 schema，而不是直接定稿。
- 记录用户选择、修改意见和未来迁移风险。

Schema 产物可以先进入 proposal；用户确认后再写入 `.agent-context/wiki/schema.md`，并同步更新 wiki maintenance skill。

后续实现应把 schema discovery 升级为 Project Design packet，统一生成候选：

```text
.agent-context/project-design/<id>/profile-candidates.md
.agent-context/project-design/<id>/schema-candidates.md
.agent-context/project-design/<id>/skill-candidates.md
.agent-context/project-design/<id>/user-decisions.md
```
...
具体流程和验收实验以 `EXTRACTION_PRD.zh-CN.md` 的 Project Design 章节为准。

### 8.4 Skills、MCP 和 Tools

Skills 编码可重复的项目工作流。它们应尽可能跨 agent 可移植，agent-specific 的部分只作为边缘适配层存在。

AgentMind 应区分 **skill content** 和 **harness discovery mechanics**：

- Skill content 是 cross-harness capability：包括 instructions、triggers、workflow steps、validation、failure handling 和 safety boundaries。
- Harness discovery 是 adapter-specific：Claude Code 可以发现 `.claude/skills/*/SKILL.md`；Codex 可能需要 `AGENTS.md` 指令和生成的 skill index；Cursor 可能需要 `.cursor/rules`。

因此 canonical skill source 应首先存在于 AgentMind：

```text
.agent-context/skills/<skill-id>/SKILL.md
```

Adapters 可以从 canonical skill 渲染出 harness-native views：
...
Claude Code 在被明确指示时可以使用 `.claude/skills` 之外的 skills，但 `.claude/skills` 给它更强的原生发现入口。Codex 在 `AGENTS.md` 或 Codex skill index 指向 canonical skill file 时，也可以使用同一个 `SKILL.md` 内容。
...
AgentMind 应把 `.agent-context/skills/` 作为项目 skills 的事实源。Adapter outputs 是 views。

渲染规则：

- Canonical skill edits 通过 proposals 或用户显式动作发生在 `.agent-context/skills/<skill-id>/SKILL.md`。
- Claude Code adapter 可以把 generated copy 渲染到 `.claude/skills/<skill-id>/SKILL.md`，用于原生发现。
- Codex adapter 应渲染 skill index，例如 `.agent-context/adapters/codex/SKILLS.md`，并在 `AGENTS.md` 中引用它。
- Adapter views 应标记为 generated，不应成为 canonical source。
- 导入的 Claude Code skills 可以先 promote 成 canonical AgentMind skills，再渲染回 Claude Code，并暴露给 Codex。

这样，一个最初为 Claude Code 写的 skill 可以成为 cross-harness capability，而不要求 Codex 拥有同样的原生 skill 目录或 hook 机制。

### 11.4 Skill Import, Promote, And Render Timing

Cross-harness skill migration 应发生在明确生命周期点。AgentMind 不应在无关工作中临时把 Claude Code skill 复制进 Codex 或 canonical storage。

| 时机 | 输入 | 动作 | 输出 |
|---|---|---|---|
| Setup existing repo | 既有 `.claude/skills/*`、`AGENTS.md`、`.cursor/rules`、本地 skill folders | Discover candidate skills，并记录 provenance | Candidate capabilities + proposals |
| Explicit import | 用户运行 `agentmind import skill <path>` | 把 source copy 或 normalize 到 AgentMind candidate storage | Candidate canonical skill + review proposal |
| Review / promote | 用户接受 proposal | 把 candidate promote 到 canonical `.agent-context/skills/<skill-id>/SKILL.md` | Active canonical skill |
| Render adapters | `connect`、`render` 或 accepted promotion | 从 canonical skill 生成 harness views | `.claude/skills/*`、Codex `SKILLS.md`、Cursor rules |
| Work end / reflection | Episode 表明有新 workflow 或 workflow 变化 | 生成 skill patch proposal | Pending skill update |

硬规则：

- Discovery 不是 promotion。
- Import 不是 activation。
- Promotion 更新 canonical skill source。
- Render 只创建 adapter views。
- Adapter views 是 generated outputs，不是事实源。
...
URL 正文抓取、正文清洗和网页转 Markdown 不应作为 AgentMind 核心从零实现。互联网上已有大量公共 skills/tools/MCP 可以承担这层能力，例如 Readability/Defuddle 类正文抽取、Trafilatura、Firecrawl、Jina Reader、MarkItDown、browser/Playwright skills，以及通用 web fetch MCP。

AgentMind 的职责是把这些能力作为 **reference intake capability** 接入，而不是把产品做成 crawler。推荐边界：

- `reference add <url>` 只登记 URL、原因、source record 和 extraction proposal。
- 如果 workspace 中存在 web-reader、browser、defuddle、Firecrawl、Jina Reader 等 capability，agent 可以调用它抓取正文并写入 `.agent-context/sources/external/` snapshot。
- AgentMind 记录 provenance、抓取工具、时间、权限、risk、hash/metadata 和关联 proposal。
- 抓取正文不等于 fixed knowledge。后续仍需 extraction flow 判断哪些内容能 promote 成 wiki、memory、skill 或 tool rule。
- 对需要登录、付费墙、动态网页、robots/版权风险或敏感内容的 URL，应要求显式用户确认，并记录风险。

因此，URL fetch 是可插拔公共 capability；把 reference 编译成项目 fixed knowledge 和 fixed capability 才是 AgentMind 核心价值。
...
AgentMind 不能只从安装之后的新对话中学习。很多项目已经有有价值的 Codex、Claude Code、Cursor、chat、issue 或 terminal 历史。用户应该能把这些历史回填进 AgentMind，让过去的工作也能转成 episodes、wiki knowledge、memory、skills 和 capability proposals。

History Backfill 通过可审核的导入流水线，把过去的对话和日志转成结构化 AgentMind assets。

### 16.1 支持输入

第一批输入类型：

- Markdown、JSON、JSONL 或纯文本格式的导出聊天记录。
- 可获取时的 Claude Code / Codex 本地 conversation exports。
- 既有 project handoff notes、`JOURNAL.md`、`CLAUDE.md`、`AGENTS.md` 和 session logs。
- 过去工作的 terminal logs 或 command transcripts。
- 用户显式提供或连接后的 GitHub issues、PR discussions 和 review comments。
...
- `capture history`：把原始 transcript/log 作为 raw source 存到 `.agent-context/sources/history/`。
- `segment episodes`：把历史拆成 work episodes，包含 goal、actions、decisions、outcomes 和 follow-up feedback。
- `extract signals`：识别持久项目事实、用户偏好、重复 workflow、失败路线、工具使用和候选 skills。
- `propose assets`：为 memory、wiki pages、wiki schema、skills、tools、work items 或 references 创建 pending proposals。
- `review/promote`：用户 accept、edit、reject 或 defer proposals 后，稳定资产才变化。
...
MVP 中，`history import <file>` 可以先支持本地 Markdown/text/JSON exports。它应创建 raw source records 和 pending proposals，而不是直接修改稳定 memory/wiki/skills。

### 16.5 安全规则

- 导入历史是 evidence，不是 truth。稳定资产仍必须通过 proposals。
- Importer 应保留 raw transcripts 和 source provenance。
- 敏感内容 promote 到 public memory 或 reusable skills 前应被标记。
- 当历史与当前代码或 wiki 矛盾时，创建 contradiction/staleness proposals，而不是覆盖。
- Backfilled skills 应先作为 candidate capabilities，直到在当前项目上下文中验证。
...
用户或 agent 可以导入过去的 conversations 和 logs，用已有项目历史 bootstrap AgentMind。

预期 workflows：

- `history import <path-or-url>`：把过去 transcript、chat export 或 session log 捕获为 raw source。
- `history segment <history-id>`：把导入历史拆成 candidate episodes。
- `history extract <history-id>`：为 memory/wiki/skills/tools/work items 生成 proposals。
- `review`：接受、编辑、拒绝或延后生成的 proposals。
...
- History backfill metadata 和 local transcript capture。
...
- 基础 `history import` 命令，支持本地 transcript 文件，生成 raw source records 和 pending extraction proposals。
...
当前代码实现了一个 CLI-first、file-backed、Codex/Claude Code adapter 的 AgentMind MVP。它不是 daemon，也不会替代 local agent；它通过生成 entry files、skills、manual、MCP 和本地状态文件指导 agent 工作。

已实现并应保持可用：

- `agentmind setup/doctor/init/status`：安全初始化 `.agent-context/`，连接 Codex 和 Claude Code。已有 repo 会创建 agent-led scan，不硬编码扫描路径。
- Online/work lifecycle：`online start/status/end`、`work add/list/claim/checkpoint/pause/finish/abandon`、`spec create`。
- Handoff 到 evolution loop：`work finish/pause` 会产生 episode 和 pending proposal。
- Reference/history capture：`reference add`、`history import`、`sources list`，用于把重要 URL、本地参考、历史对话作为 raw sources 进入 extraction loop。
- Repository scan：`scan create/list/add-source/finish`，agent 负责判断哪些路径重要，并写入 source inventory 和 extraction proposal。
- Cross-harness skills：canonical skill 位于 `.agent-context/skills/<skill-id>/SKILL.md`；`skill discover --from claude` 发现候选；`skill promote` 晋升成 canonical；`skill render` 生成 Codex index 和 Claude Code skill views。
- MCP basic tools：查询 public memory、搜索 wiki、列 skills/proposals/sources/scans/capabilities、记录 episode/reward。
- Agent manual：`.agent-context/AGENT_MANUAL.md` 指导 agent 进入项目后自动 start、查看 scans/sources/skills、创建/claim work、checkpoint、handoff。

仍是 TODO，不应在产品描述中假装已完成：

- 自动从 Codex/Claude 本地数据库批量抽取历史对话。
- Reference fetch capability adapter：复用 web-reader/browser/defuddle/Firecrawl/Jina Reader 等公共 skills/tools/MCP 抓取 URL 正文，并记录 provenance、snapshot、permission、risk 和 extraction proposal。
- Wiki lint 的 deterministic/LLM-assisted 规则实现。
- 增量 diff：已有 wiki/skills 存在时，如何只重新生成变动部分。
- Proposal apply：接受 proposal 后自动 patch wiki/skill/tool 文件。
- MCP/tool registry 的 permission/risk 生命周期。
- 自动验证 skill 更新有效性。

MVP 的目标验收是：把 AgentMind 安装到一个已有 repo 后，用户打开 Codex 或 Claude Code 说“开始”，agent 能读到 AgentMind manual，启动 session，汇报工作状态和 open scan，引导用户运行 repository scan 或继续 work，并在收工时写 handoff/episode/proposal。
...
- 从 backfilled history 生成的有用 proposals 数量。
- 被接受的 backfill proposals 在后续工作中被复用的比例。
...
- 第一批应支持哪些 history formats：Markdown exports、JSONL transcripts、Claude Code local state、Codex local state、GitHub discussions，还是 terminal logs？
- History backfill 中应如何检测和脱敏敏感内容？
